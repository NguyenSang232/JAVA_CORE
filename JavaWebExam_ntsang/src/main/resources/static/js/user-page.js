...
const petServiceAPI = {
  fetchDashboardData: async () => {
    try {
      ...
      const petsList = Array.isArray(petsData)
        ? petsData
        : petsData.content || [];
      const currentBoardings = Array.isArray(boardingData)
        ? boardingData
        : boardingData.content || [boardingData];
      ...
      petsList.forEach((pet) => {
        ...
        if (
          activePetIds.has(petIdStr) ||
          currentBoardings.some(
            (b) => b.petId === pet.id || b.pet?.id === pet.id,
          )
        ) {
      ...
      const activeSendingCount =
        activePetIds.size > 0
          ? activePetIds.size
          : currentBoardings.length > 0 && currentBoardings[0].id
            ? 1
            : 0;
      return {
  ...
  fetchHistoryData: async () => {
    try {
      ...
      const filteredHistory = historyList.filter((item) => {
        ...
        return (
          status === "RETURNED" || status === "ĐÃ TRẢ" || status === "COMPLETED"
        );
      });
  ...
  fetchBoardingDetail: async (boardingId) => {
    try {
      const response = await fetch(`${API.boarding}/${boardingId}`, {
        method: "GET",
        credentials: "include",
      });
      return response.ok ? await response.json() : null;
...
const renderTemplates = {
  pets: async () => {
    ...
    if (data.petsList.length === 0) {
      ...
      data.petsList.forEach((pet, index) => {
        ...
        rowsHTML += `
                    <tr class="pet-row" data-name="${(pet.name || pet.petName || "").toLowerCase()}">
                        <td>${index + 1}</td>
  ...
  sending: async () => {
    ...
    const noteData = await petServiceAPI.fetchCareNoteData(
      pet.id || pet.boardingId,
    );
    const notesList = noteData.careNoteList;

    const checkInStr = pet.checkInDate || pet.checkIn;
    const expectedStr =
      pet.expectedReturn || pet.expectedDate || pet.returnDate;
    ...
    if (checkInStr) {
      ...
      daysElapsed =
        Math.max(0, Math.floor((now - checkInDate) / (1000 * 60 * 60 * 24))) +
        1;
    }
    ...
    estimatedFee =
      pricePerDay > 0
        ? daysElapsed * pricePerDay
        : pet.totalFee || pet.estimatedFee || 0;
    ...
    if (data.historyList.length === 0) {
      ...
      data.historyList.forEach((item, index) => {
        ...
        if (checkInStr) {
          ...
          const endDate = isBoarding
            ? new Date()
            : new Date(item.actualCheckOut || item.returnDate);
        ...
        const totalDaysDisplay = calculatedDays
          ? calculatedDays + " ngày"
          : "N/A";
        const totalFeeDisplay = item.totalFee
          ? item.totalFee.toLocaleString("vi-VN") + "đ"
          : isBoarding
            ? "Đang cập nhật"
            : "0đ";

        rowsHTML += `
                    <tr class="history-row" data-name="${(item.petName || "").toLowerCase()}">
                        <td>${index + 1}</td>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function attachDynamicEvents() {
    // Xử lý tìm kiếm động trong bảng
    ...
    document.querySelectorAll(".btn-view-boarding").forEach((btn) => {
      btn.addEventListener("click", async function () {
        ...
        if (isBoarding) {
          alert(
            "Phiếu gửi này đang trong trạng thái gửi, không phải trạng thái Return!",
          );
          return;
        }
        ...
        modalBody.innerHTML = `
                    <div class="modal-detail-header">
                        ...
                        <span class="modal-badge completed">Return</span>
                    </div>
    ...
    document.querySelectorAll(".btn-view-notes").forEach((btn) => {
      btn.addEventListener("click", async function () {
        ...
        let notesHtml =
          notes.length === 0
            ? `<p style="color:#64748b;">Chưa có ghi chú chăm sóc nào.</p>`
            : notes
                .map(
                  (n) => `
